...
func TestValidateCatalogRejectsTemplatedGitHubURL(t *testing.T) {
	root := t.TempDir()
	pkgDir := filepath.Join(root, catalog.PackagesDir, "foo")
	if err := os.WriteFile(filepath.Join(root, "tags.yaml"), []byte("cli: command-line tool\n"), 0644); err != nil {
		t.Fatal(err)
	...
	manifest := "id: foo\n" +
		...
		"tags: [cli]\n" +
		"sources:\n" +
		"  - url: https://github.com/foo/bar/releases/download/{version}/bar.tar.gz\n" +
		"    sha256: " + strings.Repeat("a", 64) + "\n" +
		"    update:\n" +
		"      type: github\n" +
		"      repo: foo/bar\n" +
	...
	if _, err := validateCatalog(root); err == nil {
		t.Fatal("expected {version}-templated github source url to be rejected")
	}
}
