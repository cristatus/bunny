...
func TestNestedHardenedChildOnlyRemovesAccess(t *testing.T) {
	hostHome := t.TempDir()
	keep := filepath.Join(hostHome, "keep")
	revoked := filepath.Join(hostHome, "revoked")
	demoted := filepath.Join(hostHome, "demoted")
	for _, dir := range []string{keep, revoked, demoted} {
		if err := os.MkdirAll(dir, 0755); err != nil {
			t.Fatal(err)
		}
	}
	parent := sandboxContext{
		Packages: []string{"editor"}, HostHome: hostHome, Boundary: "hardened",
		NetMode: "none", FSRead: []string{keep, revoked}, FSWrite: []string{demoted},
	}
	p := &Prepared{
	...
	// Same policy: nothing to enforce, the child inherits the boundary.
	same, err := buildSandboxPlan(p, finalized(t, &PackageSandbox{Boundary: "hardened"}), "/work", "", parent)
	if err != nil {
		t.Fatal(err)
	}
	if same.needsLayer {
		t.Fatalf("child changing nothing must inherit the hardened boundary: %+v", same.args)
	}
	if !slices.Equal(same.context.FSRead, parent.FSRead) || !slices.Equal(same.context.FSWrite, parent.FSWrite) {
		t.Errorf("inherited grants lost: %+v", same.context)
	}

	// Removing and demoting grants produces masks and read-only rebinds; a
	// path absent from the parent context never becomes a grant.
	widened := filepath.Join(hostHome, "widened")
	if err := os.MkdirAll(widened, 0755); err != nil {
		t.Fatal(err)
	}
	narrower, err := buildSandboxPlan(p, finalized(t, &PackageSandbox{
		Boundary: "hardened",
		FS: FSPolicy{
			Read: []string{keep, demoted, widened}, ReadSet: true,
			Write: []string{}, WriteSet: true,
		},
	}), "/work", "", parent)
	if err != nil {
		t.Fatal(err)
	}
	if !narrower.needsLayer {
		t.Fatal("shrinking grants must add an enforcing layer")
	}
	if indexSequence(narrower.args, []string{"--tmpfs", revoked}) < 0 {
		t.Errorf("revoked read grant must be masked: %v", narrower.args)
	}
	if indexSequence(narrower.args, []string{"--ro-bind", demoted, demoted}) < 0 {
		t.Errorf("revoked write grant must demote to read-only: %v", narrower.args)
	}
	if slices.Contains(narrower.context.FSRead, widened) {
		t.Errorf("child must not add access absent from the parent context: %+v", narrower.context)
	}
}

// A hardened child launched inside an already-hardened parent must still get
// its own ephemeral overlay: the parent's boundary covers the parent's own
// home, never this child's, so buildNestedHardenedPlan cannot simply inherit
// its way past the discard guarantee.
func TestNestedHardenedChildEnforcesOwnEphemeralHome(t *testing.T) {
	hostHome := t.TempDir()
	p := &Prepared{
	...
	parent := sandboxContext{Packages: []string{"editor"}, HostHome: hostHome, Boundary: "hardened", NetMode: "none"}
	policy := finalized(t, &PackageSandbox{Boundary: "hardened", Home: "ephemeral"})
	plan, err := buildSandboxPlan(p, policy, "/work", "", parent)
	if err != nil {
		t.Fatal(err)
	}
	if !plan.needsLayer {
		t.Fatal("a nested ephemeral home must always add its own enforcing layer")
	}
	if indexSequence(plan.args, []string{"--overlay-src", home, "--tmp-overlay", home}) < 0 {
		t.Errorf("nested hardened child missing its own ephemeral overlay: %v", plan.args)
	}
}

// A clean home carries none of ephemeral's nested caveats — it never
// depends on writable access to anything — but it is still a property of
// this specific package's own home, so a nested child must add its own
// tmpfs too rather than inheriting whatever the parent's boundary exposes.
func TestNestedHardenedChildEnforcesOwnCleanHome(t *testing.T) {
	hostHome := t.TempDir()
	p := &Prepared{
		Manifest: &manifest.Manifest{ID: "node-22"},
		BinPath:  "/opt/node/bin/node",
	...
	if !plan.needsLayer {
		t.Fatal("a nested clean home must always add its own enforcing layer")
	}
	if indexSequence(plan.args, []string{"--tmpfs", home}) < 0 {
		t.Errorf("nested hardened child missing its own clean tmpfs: %v", plan.args)
	}
}

// Persist cannot be honored under a nested hardened boundary: the parent
// only ever exposes its own data directory as writable, never this child's,
// so the durable source a persist bind would rebind is at best read-only
// here. This must fail outright rather than silently mount a bind whose
// writes never survive.
func TestNestedHardenedChildRejectsPersist(t *testing.T) {
	hostHome := t.TempDir()
	p := &Prepared{
		Manifest: &manifest.Manifest{ID: "node-22"},
		BinPath:  "/opt/node/bin/node",
	...
	if _, err := buildSandboxPlan(p, policy, "/work", "", parent); err == nil {
		t.Fatal("nested hardened persist must be rejected, not silently ineffective")
	}
}

func TestScopedParentHardenedChildEscalates(t *testing.T) {
	p, hostHome := hardenedPrepared(t)
	parent := sandboxContext{Packages: []string{"editor"}, HostHome: hostHome, NetMode: "host"}
	plan, err := buildSandboxPlan(p, finalized(t, &PackageSandbox{Boundary: "hardened"}), "/work", "", parent)
	if err != nil {
		t.Fatal(err)
	}
	if !plan.needsLayer || indexSequence(plan.args, []string{"--ro-bind", "/", "/"}) < 0 {
		t.Fatal("a hardened child under a scoped parent must build the full hardened layer")
	}
	if plan.context.Boundary != "hardened" {
		t.Errorf("context must escalate to hardened: %+v", plan.context)
	}
}

func TestHardenedParentKeepsScopedChildHardened(t *testing.T) {
	hostHome := t.TempDir()
	parent := sandboxContext{
		Packages: []string{"editor"}, HostHome: hostHome,
	...
	plan, err := buildSandboxPlan(p, finalized(t, &PackageSandbox{Home: "isolated"}), "/work", "", parent)
	if err != nil {
		t.Fatal(err)
	}
	if plan.context.Boundary != "hardened" {
		t.Errorf("boundary is monotonic; a scoped child stays hardened: %+v", plan.context)
	}
}

// The hardened baseline makes /run private, which on a systemd-resolved host
// leaves /etc/resolv.conf (a symlink into /run) dangling and breaks every name
// lookup. Host networking gains nothing from hiding it, so it is bound back;
// a restricted mode keeps it masked.
// Bunny's own layout is bound back read-only, so a shim inside the boundary
// can still resolve a package. Read-only is the whole point: it keeps
// re-entry working without letting the package install, uninstall, or rewrite
// state.json from inside.
func TestHardenedBindsBunnyLayoutReadOnly(t *testing.T) {
	p, hostHome := hardenedPrepared(t)
	layout := t.TempDir()
	shims := filepath.Join(layout, "bin")
	state := filepath.Join(layout, "state.json")
	sdkRoot := filepath.Join(layout, "custom-sdk-root")
	for _, dir := range []string{shims, sdkRoot} {
		if err := os.MkdirAll(dir, 0755); err != nil {
			t.Fatal(err)
		}
	}
	if err := os.WriteFile(state, []byte("{}"), 0644); err != nil {
		t.Fatal(err)
	}
	p.LayoutRoots = []string{shims, state, sdkRoot}
	...
	dataAt := indexSequence(plan.args, []string{"--bind", p.Vars["data"], p.Vars["data"]})
	for _, path := range p.LayoutRoots {
		at := indexSequence(plan.args, []string{"--ro-bind", path, path})
		if at < 0 {
			t.Errorf("layout path %s must be readable for shim re-entry: %v", path, plan.args)
			continue
		}
		if at < dataAt {
			t.Errorf("layout path %s must mount after the package data bind", path)
		}
		if indexSequence(plan.args, []string{"--bind", path, path}) >= 0 {
			t.Errorf("layout path %s must not be writable: %v", path, plan.args)
		}
	}
}

// An install root may legally be set to $HOME. Binding it back would undo the
// home mask and hand the package the entire host home, so the grant is
